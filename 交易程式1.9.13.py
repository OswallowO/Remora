...
class VolumeAnalysisDialog(BaseDialog):
    def __init__(self):
        ...
        self.params_config = {
            ...
            'min_lag_pct':        {'name': '落後領漲幅 (%)', 'fixed': 1.0, 'start': 0.0, 'end': 2.0, 'step': 0.1, 'is_int': False},
            'min_height_pct':     {'name': '最高漲幅門檻 (%)', 'fixed': 3.0, 'start': 0.0, 'end': 3.5, 'step': 0.1, 'is_int': False},
            'volatility_min_range': {'name': '漲跌幅活動範圍 (%)', 'fixed': 1.5, 'start': 0.0, 'end': 3.0, 'step': 0.1, 'is_int': False},
            'min_eligible_avg_vol': {'name': '全日均量下限 (張/分)', 'fixed': 1, 'start': 0, 'end': 5, 'step': 1, 'is_int': True}
        }
